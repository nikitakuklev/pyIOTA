...
from ocelot import Hcor, Vcor, Quadrupole
...
class IOTAOptimizer(Optimizer):
    ...
    def add_corrector_variables(self, box: LatticeContainer = None, correctors: list = None):
        """
        Adds a kick strength variable for all correctors
        :param correctors:
        :param box:
        :return:
        """
    ...
    def add_corrector_constraints(self, box: LatticeContainer = None, max_kicks: list = None, min_kicks: list = None,
        ...
        for i, c in enumerate(box.correctors):
            ...
            self.add_term(f"{c.ref_el.id}#0.{item} {max_kicks[i]} {tol} segt")
            self.add_term(f"{c.ref_el.id}#0.{item} {min_kicks[i]} {tol} selt")
            print(f'Added {c.ref_el.id}-{item} constraint:({max_kicks[i]}|{min_kicks[i]})@{tol}')

    def add_orbit_constraints(self, box: LatticeContainer = None, goals: Union[list, dict] = None, tol: float = 1e-5,
        ...
        if isinstance(goals, list):
            ...
            for m, (x, y) in zip(box.monitors, goals):
                self.add_term(self.sene(f'{m.id}#0.xco', x, tol))
                self.add_term(self.sene(f'{m.id}#0.yco', y, tol))
            ...
            if zero_other_monitors:
                for m in monitors_other:
                    self.add_term(self.sene(f'{m.id}#0.xco', str(0), tol))
                    self.add_term(self.sene(f'{m.id}#0.yco', str(0), tol))
                print(f'Added {len(monitors_other)+len(goals)} orbit constraints, ({len(monitors_other)} by default on reference orbit)')
            else:
                print(f'Added {len(goals)} orbit constraints, ({len(monitors_other)} unused)')
