from ocelot import Element, Sextupole, MagneticLattice, Octupole, twiss, Drift
import numpy as np


class LatticeContainer:
    def __init__(self, name: str, lattice: list, correctors: list,
                 monitors: list, reset_elements_to_defaults: bool = True):
        self.name = name
        ...
        if reset_elements_to_defaults:
            print(f'WARN - resetting any multipoles to 0')
    ...
    def insert_correctors(self, destroy_skew_quads=True):
    ...
    def insert_monitors(self, monitors: list = None, verbose: bool = False):
        ...
        for m in monitors:
            ...
            if m.s in s or np.any(np.isclose(m.s, s)):
                ...
                # print(i,m.s)
                if verbose:print(f'{m.id} - inserted at {m.s}(idx {i}) between {self.lattice.sequence[i - 1].id} and {self.lattice.sequence[i].id}')
                ...
                if verbose:print(f'{m.id} - location {m.s} in collision with ({occupant.__class__.__name__} {occupant.id})(sidx:{occupant_s_idx}) - length {occupant.l}m, between {s[max(occupant_s_idx - 1, 0):min(occupant_s_idx + 3, len(s) - 1)]}')
                if isinstance(occupant, Drift):
                    ...
                    if verbose:print(f'{m.id} - inserted at {m.s}, two new drifts {d_before.l} and {d_after.l} created')
                    self.lattice.update_transfer_maps()
                    if self.lattice.totalLen != len_before:
                        raise Exception(f"Lattice length changed from {len_before} to {self.lattice.totalLen}!!!")
                    b+=1
                else:
                    if verbose:print(f'Could not place monitor {m.id} at {m.s} - collision with {occupant.__class__.__name__} {occupant.id} - length {occupant.l}m, closest edges ({np.max(s[s < m.s])}|{np.min(s[s > m.s])})')
                    c+=1
        print(f'Inserted {a} cleanly, {b} with drift splitting, {c} rejected')

    def get_elements(self, el_type=None):
    ...
    def get_first(self, el_type=None):
        if isinstance(el_type, str):
            return [el for el in self.lattice.sequence if el.__class__.__name__ in el_type][0]
        else:
            return [el for el in self.lattice.sequence if isinstance(el, el_type)][0]
